...
| `title` | string | 是 | 节点标题 |
| `desc` | string | 否 | 节点描述 |
| `model` | object | 是 | 模型配置：`provider`、`name`、`mode`（`"chat"`/`"completion"`）、`completion_params`（可选） |
| `prompt_template` | array | 是 | 提示词消息列表，每条含 `role`（`"system"`/`"user"`/`"assistant"`）、`text`，可选 `id` |
| `context` | object | 否 | 知识库上下文，`{ enabled: boolean, variable_selector: string[] }`，默认 `{ enabled: false, variable_selector: [] }` |
| `vision` | object | 否 | 视觉输入，`{ enabled: boolean }`，默认 `{ enabled: false }` |
| `memory` | object | 否 | 对话记忆（仅 advanced-chat 模式），含 `window`、`query_prompt_template`、可选 `role_prefix` |
| `prompt_config` | object | 否 | Jinja2 变量配置，`{ jinja2_variables: unknown[] }` |
| `position` | {x, y} | 否 | 画布坐标 |
...
### 3.6 `add-classifier-class` — 给分类器新增分类
...
### 3.11 `set-answer` — 修改 Answer 节点模板
...
### 3.13 `set-start-var` — 修改 Start 节点变量字段
...
### 3.17 `update-condition` — 修改 if-else 条件 (v1.0.4+)
...
### 3.18 `remove-classifier-class` — 删除分类器 class (v1.0.4+)
