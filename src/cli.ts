...
function nodeLabel(n: any, useShort: boolean): string {
  const t = TYPE_LABEL[n.data.type] || n.data.type;
  const id = useShort ? n.id.slice(-6) : n.id;
  return `[${t}] ${id} ${n.title || ""}`;
}

function cmd_flow(args: string[]) {
  ...
  const roots: string[] = [];
  for (const n of dsl.index.byId.values()) {
    if (dsl.getPrevIds(n.id).length === 0 && !n.parentId) roots.push(n.id);
  }

  const visited = new Set<string>();

  function getBranches(id: string): { handle: string; target: string }[] {
  ...
  function tree(targetIds: string[] | string, prefix: string, isLast: boolean[]) {
...
function cmdNodeShow(args: string[]) {
  ...
  if (n.parentId) console.log(`Parent:   ${n.parentId}`);
  if (n.isInIteration) console.log(`InIter:   ${n.iterationId}`);
  ...
  switch (type) {
    ...
    case "code": {
      ...
      if (data.outputs) {
        console.log(`\n-- Outputs --`);
        for (const [name, out] of Object.entries(data.outputs as Record<string, any>)) {
    ...
    case "start": {
      ...
      for (const v of data.variables || []) {
        ...
        if (v.options?.length) console.log(`    options: [${v.options.join(", ")}]`);
        if (v.placeholder) console.log(`    placeholder: ${v.placeholder}`);
      }
      break;
    }
    case "answer": {
    ...
    case "tool": {
      ...
      if (data.tool_parameters) {
        console.log(`\n-- Parameters --`);
        for (const [k, v] of Object.entries(data.tool_parameters as Record<string, any>)) {
    ...
    case "http-request": {
      ...
      if (data.body?.type !== "none") console.log(`  body data: ${data.body?.data?.slice(0, 200) || ""}`);
      break;
    }
...
function cmdFind(args: string[]) {
  ...
  for (const n of dsl.index.byId.values()) {
    ...
    function check(field: string, text: string) {
      if (!text) return;
    ...
    if (d.type === "code") check("code", d.code);
    if (d.type === "answer") check("answer", d.answer);
    if (d.type === "template-transform") check("template", d.template);
    if (d.type === "start") {
      for (const v of d.variables || []) {
        if (v.label) check("variable.label", v.label);
        if (v.placeholder) check("variable.placeholder", v.placeholder);
      }
    }
  }
...
async function main() {
  ...
  switch (cmd) {
    ...
    case "info":
      ...
      break;
    case "flow":
      if (args.length < 1) fail("Usage: dify-dsl-cli flow <file> [--short]");
      cmd_flow(args);
      break;
    case "find":
