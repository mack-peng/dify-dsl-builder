...
import { DifyDSL, Edge } from "./index";
...
function cmd_info(args: string[]) {
  ...
  for (const n of (dsl.graph as any)._nodes.values()) {
    const t = n.data?.type || "?";
...
function atomEdgeAdd(args: string[]) {
  atomicNode(args, 0, (dsl) => {
    dsl.graph.addEdge(new Edge(args[1], args[2], args[3] || "source"));
  });
}
...
function atomEdgeRemove(args: string[]) {
  atomicNode(args, 0, (dsl) => {
    ...
    if (dsl.graph.edgeCount === before) fail(`Edge not found: ${id}`);
  });
}
