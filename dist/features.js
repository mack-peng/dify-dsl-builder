    ...
    toYAML(w) {
        ...
        w.indent(() => {
            ...
            w.indent(() => {
                ...
                w.indent(() => f.file_upload.allowed_file_extensions.forEach(e => w.raw(`- ${e}`)));
                w.key("allowed_file_types");
                w.indent(() => f.file_upload.allowed_file_types.forEach(t => w.raw(`- ${t}`)));
                w.key("allowed_file_upload_methods");
                w.indent(() => f.file_upload.allowed_file_upload_methods.forEach(m => w.raw(`- ${m}`)));
                ...
                w.indent(() => {
                    ...
                    w.indent(() => f.file_upload.image.transfer_methods.forEach(m => w.raw(`- ${m}`)));
                });
            ...
            w.indent(() => w.keyVal("enabled", f.retriever_resource.enabled));
            w.key("sensitive_word_avoidance");
            w.indent(() => w.keyVal("enabled", f.sensitive_word_avoidance.enabled));
            w.key("speech_to_text");
            w.indent(() => w.keyVal("enabled", f.speech_to_text.enabled));
            w.key("suggested_questions");
            w.indent(() => f.suggested_questions.forEach(q => w.raw(`- ${q}`)));
            w.key("suggested_questions_after_answer");
            w.indent(() => w.keyVal("enabled", f.suggested_questions_after_answer.enabled));
